...
// primary query joins on the food catalog.
function buildAllergenRows(entries: ReportEntry[]): AllergenReportRow[] {
  ...
  return ALLERGENS.map((a): AllergenReportRow => {
...
function computeMostAdvancedTexture(entries: ReportEntry[]): TextureKey | null {
  ...
  return mostAdvancedIdx >= 0 ? TEXTURE_VALUES[mostAdvancedIdx] : null;
}

// 30-day texture distribution: count each texture key logged in the window.
function computeTextureDistribution(entries: ReportEntry[]): {
...
export const load: PageServerLoad = async ({ parent, locals, params }) => {
  ...
  const allergens = buildAllergenRows(entries);
  ...
  const mostAdvancedTexture = computeMostAdvancedTexture(entries);
