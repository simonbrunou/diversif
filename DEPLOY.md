...
3. **Verify migration success** before considering the deploy done:
   ...
   - Sentry → Crons → `diversif-cleanup` shows a fresh **ok** check-in for the
     new release (it also runs once at boot, so this confirms the new
     container came up cleanly).
   - Sentry → Logs shows a **"Server starting"** entry tagged with the new
     release (git SHA), confirming the deployed build is the one logging.
...
| `SENTRY_DSN`, `PUBLIC_SENTRY_DSN` (+ `SENTRY_ENVIRONMENT`, `SENTRY_RELEASE`, `SENTRY_TRACES_SAMPLE_RATE`, `PUBLIC_SENTRY_TRACES_SAMPLE_RATE`, `PUBLIC_SENTRY_REPLAYS_SESSION_SAMPLE_RATE`, `PUBLIC_SENTRY_REPLAYS_ON_ERROR_SAMPLE_RATE`) | Errors, sampled performance tracing, server logs, the `diversif-cleanup` cron monitor, and error-only masked session replay all reach Sentry instead of dying silently; an empty `PUBLIC_SENTRY_DSN` also disables the `/monitoring` tunnel and browser-side capture.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
- [ ] **O1 — Self-hosted error monitoring (optional, data-sovereignty).** The app
      ...
      instance (its DSN's path prefix is honoured by the `/monitoring` tunnel,
      so no CSP change is needed — the browser only ever talks to Diversif's
      own origin). No hook/SDK changes needed. GlitchTip covers error capture
      but does not support Sentry's Session Replay, Logs, or Crons, so the
      `diversif-cleanup` check-in, the operational log lines, and error-only
      session replay would have no destination until/unless GlitchTip adds
      those features.
