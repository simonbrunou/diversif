# Graph Report - diversif  (2026-05-11)

## Corpus Check
- 352 files · ~406,921 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1672 nodes · 2279 edges · 220 communities (161 shown, 59 thin omitted)
- Extraction: 83% EXTRACTED · 17% INFERRED · 0% AMBIGUOUS · INFERRED: 397 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `2ae33424`
...
## Communities (220 total, 59 thin omitted)
...
Cohesion: 0.07
Nodes (43): load(), load(), POST(), createSession(), findUserByEmail(), getDecoyHash(), invalidateAllUserSessions(), invalidateSession() (+35 more)
...
Cohesion: 0.18
Nodes (14): load(), allergens load test, load(), loadEntry(), parseEntryId(), load(), load(), load() (+6 more)

### Community 9 - "WebAuthn / Passkeys"
Cohesion: 0.13
Nodes (13): filterIncomingBreadcrumb(), scrubEvent(), scrubPathname(), scrubUrlString(), enforceLanguageTag(), setLanguageTag(), handleError(), formatDateInputValue() (+5 more)
...
Cohesion: 0.16
Nodes (12): bentoEnabled(), absoluteUrl(), articleJsonLd(), breadcrumbJsonLd(), faqPageJsonLd(), organizationJsonLd(), resolveOrigin(), webApplicationJsonLd() (+4 more)
...
Cohesion: 0.24
Nodes (6): findActiveInvitation(), load(), userHasMembership(), POST(), load(), localizedRedirect()

### Community 21 - "Diversification Guidance & Stages"
Cohesion: 0.25
Nodes (6): isUniqueViolation(), generateUniqueInviteCode(), insertInviteWithUniqueCode(), load(), generateInviteCodeRaw(), isValidInviteCodeFormat()
...
- **Why does `languageTag()` connect `i18n Messages (auth.*)` to `Community 64`, `Community 65`, `WebAuthn / Passkeys`?**
  _High betweenness centrality (0.079) - this node is a cross-community bridge._
- **Why does `setLanguageTag()` connect `WebAuthn / Passkeys` to `i18n Messages (auth.*)`, `Page Loaders & Form Actions`?**
  _High betweenness centrality (0.047) - this node is a cross-community bridge._
- **Why does `validateSession()` connect `Page Loaders & Form Actions` to `Test Fixtures & Setup Helpers`?**
  _High betweenness centrality (0.038) - this node is a cross-community bridge._
- **Are the 242 inferred relationships involving `languageTag()` (e.g. with `localizedHref()` and `formatRelative()`) actually correct?**
  _`languageTag()` has 242 INFERRED edges - model-reasoned connections that need verification._
