...
## Corpus Check
- 273 files · ~369,652 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1543 nodes · 2134 edges · 181 communities (127 shown, 54 thin omitted)
- Extraction: 83% EXTRACTED · 17% INFERRED · 0% AMBIGUOUS · INFERRED: 371 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9f34ec6b`
...
9. `localizedRedirect()` - 18 edges
10. `seedMembership()` - 18 edges
...
Cohesion: 0.02
Nodes (230): authAccountAppearanceDescription(), authAccountAppearanceSection(), authAccountBack(), authAccountCurrentPasswordLabel(), authAccountDataDescription(), authAccountDataExport(), authAccountDataSection(), authAccountDeleteConfirmLabel() (+222 more)
...
Cohesion: 0.08
Nodes (37): load(), load(), POST(), createSession(), findUserByEmail(), getDecoyHash(), invalidateAllUserSessions(), newToken() (+29 more)

### Community 5 - "Branding, App Shell & PWA Manifest"
Cohesion: 0.07
Nodes (30): load(), allergens load test, findActiveInvitation(), load(), userHasMembership(), load(), loadEntry(), parseEntryId() (+22 more)
...
Cohesion: 0.15
Nodes (10): filterIncomingBreadcrumb(), scrubEvent(), scrubPathname(), scrubUrlString(), enforceLanguageTag(), setLanguageTag(), invalidateSession(), listMembershipsForUser() (+2 more)
...
- **Why does `languageTag()` connect `i18n Messages (auth.*)` to `Offline Log Queue (IndexedDB)`, `Community 51`?**
...
- **Are the 230 inferred relationships involving `languageTag()` (e.g. with `localizedHref()` and `chromeAppShellMyAccount()`) actually correct?**
  _`languageTag()` has 230 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 11 inferred relationships involving `requireUser()` (e.g. with `localizedRedirect()` and `load()`) actually correct?**
