...
## Corpus Check
- Large corpus: 291 files · ~162,354 words. Semantic extraction will be expensive (many Claude tokens). Consider running on a subfolder, or use --no-semantic to run AST-only.

## Summary
- 1461 nodes · 1944 edges · 176 communities (122 shown, 54 thin omitted)
- Extraction: 81% EXTRACTED · 19% INFERRED · 0% AMBIGUOUS · INFERRED: 360 edges (avg confidence: 0.8)
- Token cost: 149,845 input · 16,649 output
...
1. `languageTag()` - 233 edges
2. `makeRouteEvent()` - 35 edges
3. `resetTestDb()` - 31 edges
4. `safeUser()` - 23 edges
5. `seedChild()` - 22 edges
6. `requireUser()` - 20 edges
7. `requireMembership()` - 20 edges
8. `captureFlow()` - 20 edges
9. `seedMembership()` - 18 edges
10. `seedUser()` - 15 edges
...
Cohesion: 0.02
Nodes (228): authAccountAppearanceDescription(), authAccountAppearanceSection(), authAccountBack(), authAccountCurrentPasswordLabel(), authAccountDataDescription(), authAccountDataExport(), authAccountDataSection(), authAccountDeleteConfirmLabel() (+220 more)

### Community 3 - "Page Loaders & Form Actions"
Cohesion: 0.05
Nodes (45): load(), allergens load test, load(), getStageForAgeMonths(), getTipsFor(), pickRotatingTip(), load(), loadEntry() (+37 more)

### Community 4 - "Test Fixtures & Setup Helpers"
Cohesion: 0.09
Nodes (24): seed(), loadFor(), setup(), setup(), loadFor(), setup(), setup(), setup() (+16 more)
...
Cohesion: 0.08
Nodes (24): load(), POST(), enforceLanguageTag(), setLanguageTag(), createSession(), findUserByEmail(), invalidateAllUserSessions(), invalidateSession() (+16 more)
...
Cohesion: 0.13
Nodes (22): load(), POST(), base64UrlToBuffer(), bufferToBase64Url(), buildAuthenticationOptions(), buildRegistrationOptions(), consumeChallenge(), createChallenge() (+14 more)
...
Cohesion: 0.14
Nodes (12): findActiveInvitation(), load(), userHasMembership(), load(), LogActionAbort, IdempotencyInFlight, IdempotencyScopeMismatch, pruneExpiredKeys() (+4 more)
...
Cohesion: 0.33
Nodes (4): filterIncomingBreadcrumb(), scrubEvent(), scrubPathname(), scrubUrlString()
...
- **Why does `languageTag()` connect `i18n Messages (auth.*)` to `Community 45`, `Architecture Patterns & Conventions`?**
  _High betweenness centrality (0.094) - this node is a cross-community bridge._
- **Why does `setLanguageTag()` connect `Architecture Patterns & Conventions` to `i18n Messages (auth.*)`, `Sentry Observability`?**
  _High betweenness centrality (0.081) - this node is a cross-community bridge._
- **Why does `validateSession()` connect `Architecture Patterns & Conventions` to `Test Fixtures & Setup Helpers`?**
  _High betweenness centrality (0.063) - this node is a cross-community bridge._
- **Are the 228 inferred relationships involving `languageTag()` (e.g. with `localizedHref()` and `chromeAppShellMyAccount()`) actually correct?**
  _`languageTag()` has 228 INFERRED edges - model-reasoned connections that need verification._
