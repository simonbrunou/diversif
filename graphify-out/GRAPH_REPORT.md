...
## Corpus Check
- 273 files · ~370,498 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1546 nodes · 2144 edges · 189 communities (132 shown, 57 thin omitted)
- Extraction: 83% EXTRACTED · 17% INFERRED · 0% AMBIGUOUS · INFERRED: 375 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `e8fbe2fc`
...
Cohesion: 0.09
Nodes (34): load(), POST(), createSession(), findUserByEmail(), getDecoyHash(), invalidateAllUserSessions(), newToken(), verifyPassword() (+26 more)
...
Cohesion: 0.15
Nodes (11): findActiveInvitation(), load(), userHasMembership(), load(), load(), requireGuest(), localizedRedirect(), generateUniqueInviteCode() (+3 more)
...
Cohesion: 0.33
Nodes (6): load(), formatDateInputValue(), formatDateTime(), formatRelative(), isValidBirthDate(), parseDateTimeLocal()
...
Cohesion: 0.83
Nodes (3): load(), loadEntry(), parseEntryId()
...
- **Why does `languageTag()` connect `i18n Messages (auth.*)` to `WebAuthn / Passkeys`, `Community 58`?**
  _High betweenness centrality (0.086) - this node is a cross-community bridge._
- **Why does `setLanguageTag()` connect `WebAuthn / Passkeys` to `i18n Messages (auth.*)`?**
  _High betweenness centrality (0.078) - this node is a cross-community bridge._
- **Why does `validateSession()` connect `Page Loaders & Form Actions` to `WebAuthn / Passkeys`, `Test Fixtures & Setup Helpers`?**
