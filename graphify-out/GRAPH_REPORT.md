# Graph Report - diversif  (2026-05-08)

## Corpus Check
- 268 files · ~358,702 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1528 nodes · 2083 edges · 184 communities (130 shown, 54 thin omitted)
- Extraction: 83% EXTRACTED · 17% INFERRED · 0% AMBIGUOUS · INFERRED: 361 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `4c97be17`
...
## Communities (184 total, 54 thin omitted)
...
Cohesion: 0.06
Nodes (42): load(), allergens load test, load(), load(), loadEntry(), parseEntryId(), load(), dismissReminder() (+34 more)
...
Cohesion: 0.07
Nodes (38): load(), load(), POST(), createSession(), findUserByEmail(), invalidateAllUserSessions(), newToken(), verifyPassword() (+30 more)
...
Cohesion: 0.24
Nodes (7): isUniqueViolation(), load(), LogActionAbort, IdempotencyInFlight, IdempotencyScopeMismatch, pruneExpiredKeys(), withIdempotencyKey()
...
- **Why does `languageTag()` connect `i18n Messages (auth.*)` to `WebAuthn / Passkeys`, `Community 52`?**
  _High betweenness centrality (0.074) - this node is a cross-community bridge._
- **Why does `setLanguageTag()` connect `WebAuthn / Passkeys` to `i18n Messages (auth.*)`?**
  _High betweenness centrality (0.065) - this node is a cross-community bridge._
- **Why does `validateSession()` connect `Test Fixtures & Setup Helpers` to `WebAuthn / Passkeys`, `Branding, App Shell & PWA Manifest`?**
...
- **Are the 10 inferred relationships involving `requireUser()` (e.g. with `load()` and `GET()`) actually correct?**
  _`requireUser()` has 10 INFERRED edges - model-reasoned connections that need verification._
