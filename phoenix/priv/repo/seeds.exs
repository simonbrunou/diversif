# Mirror of `src/lib/server/db/seed.ts`: the global, non-custom foods catalog.
# Idempotent via the partial unique index foods_name_seed_idx — re-running this
# script does nothing on already-seeded rows.
...
rows =
  Enum.map(foods, fn f ->
    %{
      name: f.name,
      category: f.category,
      is_major_allergen: Map.has_key?(f, :allergen),
      allergen_type: Map.get(f, :allergen),
      suggested_age_months: f.age,
      notes: nil,
      is_custom: false,
      custom_for_child_id: nil
    }
  end)

{count, _} = Repo.insert_all(Food, rows, on_conflict: :nothing)
IO.puts("seeded #{count} foods (skipped #{length(rows) - count} existing)")
