...
- In Claude Code the cross-model `run_agents` roster isn't available — subagents are Claude-only. Fall back to perspective-diverse members (distinct angles, optionally different Claude tiers via the Agent/Workflow tools) and say so briefly.
