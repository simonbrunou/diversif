...
ENV PORT=3000
ENV HOST=0.0.0.0
# Trust Cloudflare/Traefik's forwarded headers so adapter-node derives the
# request origin per-request instead of from a build-baked ORIGIN env var.
# Lets CSRF accept POSTs from every preview hostname (`*.diversif.app`)
# without per-deploy configuration. Only set these when sitting behind a
# trusted reverse proxy — clients could otherwise spoof them.
